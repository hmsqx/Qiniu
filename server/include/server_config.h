...
struct ServerConfig {
    // 线程池配置
    // 在某些受限环境下 hardware_concurrency 可能返回 0 或 1，导致近似单线程处理；
    // 这里设置一个合理的下限，保证至少有 4 个工作线程（或 CPU 核心数的两倍，取较大者）。
    size_t threadPoolSize = [](){
        auto hc = std::thread::hardware_concurrency();
        if (hc == 0) hc = 2; // 防御性：未知时按 2 处理
        size_t recommended = std::max<size_t>(4, hc);
        return recommended;
    }();
